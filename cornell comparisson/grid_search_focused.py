# =============================================================================
# FOCUSED GRID SEARCH WITH MANUAL CROPPING
# =============================================================================
# Reduced to ~100-150 combinations focusing on best-performing ranges
...
# FOCUSED weight combinations - only CoG-dominant strategies
...
# FOCUSED parameter grid - concentrating on promising ranges
PARAM_GRID = {
    "weight_combo": list(range(len(WEIGHT_COMBINATIONS_FOCUSED))),  # 3 values
    "depth_percentile": [18, 20, 22, 25, 28, 30, 32],              # 7 values (focused 18-32)
...
# Total: 3 × 7 × 1 × 3 × 2 × 2 × 1 × 3 × 1 = 252 combinations

print("=" * 80)
print("FOCUSED GRID SEARCH CONFIGURATION")
print("=" * 80)
...
for param, values in PARAM_GRID.items():
    if param == "weight_combo":
        print(f"  Weight combinations: {len(WEIGHT_COMBINATIONS_FOCUSED)}")
    else:
...
print("\n" + "=" * 80)
print("STEP 2: FOCUSED GRID SEARCH")
print("=" * 80)
...
print(f"Estimated time: {len(all_combinations) * 10 / 60:.1f} minutes")
print("=" * 80)
...
for combo_idx, combo in enumerate(all_combinations):
    ...
    detector = GraspDetector(w_edge, w_depth, w_cog)
...
print("\n" + "=" * 80)
print("FOCUSED GRID SEARCH RESULTS")
print("=" * 80)
...
for i, result in enumerate(grid_results[:10]):
    ...
    print(f"  Depth%={params['depth_percentile']}, mult={params['candidate_multiplier']}, boost={params['cog_boost']:.2f}")
    print(f"  Length: {params['min_grasp_length']}-{params['max_grasp_length']}")
...
# Save results
results_file = 'grid_search_focused_results.json'
