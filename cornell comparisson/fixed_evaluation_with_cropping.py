...
def adjust_grasp_coordinates(grasps, crop_bbox):
    ...
    Grasps are dictionaries with 'center', 'angle', 'width', 'height', 'corners'
    """
    x_offset, y_offset, crop_w, crop_h = crop_bbox
    adjusted_grasps = []

    for grasp in grasps:
        # Get center from dictionary
...
# =============================================================================
# STEP 1: PRECOMPUTE DEPTH WITH OPTIONAL CROPPING
# =============================================================================

cached_data = []
crop_stats = {'total': 0, 'cropped': 0, 'no_surface': 0, 'no_grasps': 0}
start = time.time()

for sample in tqdm(samples, desc="Computing depth maps"):
    try:
        ...
        if ENABLE_WHITE_CROP:
            ...
            if crop_success:
                ...
                if len(adjusted_grasps) == 0:
                    crop_stats['no_grasps'] += 1
                    # Fall back to original if no grasps remain
                    cropped_rgb = rgb
                    adjusted_grasps = sample["grasps"]
                else:
...
print(f"\n✓ Cached {len(cached_data)} / {len(samples)} samples")
if ENABLE_WHITE_CROP:
    print(f"  Successfully cropped: {crop_stats['cropped']}/{crop_stats['total']}")
    print(f"  No surface detected: {crop_stats['no_surface']}")
    print(f"  No grasps in crop: {crop_stats['no_grasps']}")
print(f"Depth precomputation time: {time.time() - start:.1f}s")

# =============================================================================
# STEP 2: RUN DETECTOR (cropping already done, now just mask->COG->grasp)
# =============================================================================
...
for data in tqdm(cached_data, desc="Evaluating with cropping"):
    try:
        # NOTE: Cropping already done in Step 1, so disable it here
