...
class ImprovedGraspDetector:
    ...
    def _cast_single_ray(self, mask, start_x, start_y, dx, dy, max_dist):
        ...
        # ##################33 PARAMETER: How many pixels of "background" to ignore before stopping.
        gap_tolerance = 10 
        current_gap = 0
        ...
        for _ in range(max_dist):
            ...
            if mask[iy, ix] == 0:
                ...
                # If the gap is too big, we assume we really hit the edge.
                # Return the LAST KNOWN valid point (before the gap started).
                if current_gap > gap_tolerance:
                    return last_valid_x, last_valid_y
            else:
                # We are still on the object (255). 
...
def manage_model_memory(new_model_name):
    ...
    if 'model_name' in st.session_state and st.session_state.model_name != new_model_name:
        ...
        if 'depth_wrapper' in st.session_state:
            ...
            # Clear CUDA cache (Essential for MiDaS switch otherwise it throws errors)
